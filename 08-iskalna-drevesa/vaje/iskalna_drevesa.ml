...
(*----------------------------------------------------------------------------*]
 ...
 - : bool = false
[*----------------------------------------------------------------------------*)
let rec vecji x = function
     | [] -> true
     | y :: ys -> if y < x then vecji x ys else false
let rec manjsi x = function
     | [] -> true
     | y :: ys -> if y > x then manjsi x ys else false

let rec is_bst = function
     | Empty -> true
     | Node(l,x,d) -> if (vecji x (list_of_tree(l))) && (manjsi x (list_of_tree(d))) && (is_bst l) && (is_bst d) then true else false

(*Alternativno*)
let rec is_ordered = function
     | x::y::xs -> x <= y && is_ordered(y::xs)
     | _ -> true
...
(*Alternativno2*)
let is_bst_eff =
     let je_vmes lb x up = match(lb,up) with
          | (None,None) -> true
          | (None, Some up') -> x < up'
          | (Some lb', None) -> lb' < x
          | (Some lb', Some up') -> (lb' < x) && (x < up')
     in
     let rec pomozna curr_min curr_max = function
          | Empty -> true
          | Node(l,x,d) -> (je_vmes curr_min x curr_max) && (pomozna curr_min (Some x) l) && (pomozna (Some x) curr_max d)
     in
     pomozna None None
...
let rec insert x = function
     | Empty -> leaf x
     | Node(l,y,d) -> if x < y then Node(insert x l, y, d) else Node(l, y, insert x d)

let rec member v = function
     | Empty -> false
     | Node(l,x,d) -> 
          if v < x then member x l else
          if x < v then member x d else true
(*----------------------------------------------------------------------------*]
...
let rec member2 x = function
     | Empty -> false
     | Node(l,y,d) ->
          if x = y then true else ((member2 x l) || (member2 x d))
...
let succ drevo =
     let rec pom = function
          | Empty -> None
          | Node(l,x,d) -> let a = pom l in if a = None then Some x else a
     in match drevo with
          | Empty -> None
          | Node(l,x,d) -> pom d
...
let pred drevo = 
     let rec pom = function
          | Empty -> None
          | Node(l,x,d) -> let a = pom d in if a = None then Some x else a
     in match drevo with
          | Empty -> None
          | Node(l,x,d) -> pom l
...
let rec delete e = function
     | Empty -> Empty
     | Node(l,x,d) -> if (x<e) then Node(delete e l,x,d) else if (e<x) then Node(l,x, delete e d)
          else 
(*-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*]
 SLOVARJI
